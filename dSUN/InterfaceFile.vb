...
Module InterfaceFile
    ...
    Public Async Sub buildPublish(slnn As String, path1 As String, path2 As String, projectname2 As String, netver2 As String, output2 As TextBox, appName2 As TextBox, btnDeploy2 As Button, pr2 As ProgressBar, btL As Button)
        ...
        ElseIf (result_shell.Contains("not logged in")) Then
            ...
            output.Text += "Bạn chưa đăng nhập Heroku." + vbNewLine + "Đang mở CMD tiến hành đăng nhập..." + vbNewLine + "Vui lòng thực hiện đăng nhập và thử lại sao!" + vbNewLine
        ...
        Else
            ...
            '    failStop()
            'Else
            output.Text += "Đã xảy ra lỗi, không thể thực hiện việc kiểm tra Heroku." + vbNewLine + "Hãy đảm bảo bạn đã cài đặt và đăng nhập Heroku." + vbNewLine + "Hãy thử đăng nhập lại vào Heroku bằng cách click vào 'Login Heroku' phía trên." + vbNewLine + "Vui lòng thử lại sao!" + vbNewLine
            btnLog.Visible = True
            failStop()
            ' End If
    ...
    Public Async Sub releasePr(p1 As String, p2 As String)
        ...
        If (result_shell.Length > 1) Then
            ...
            Else
                output.Text += "Release dự án thất bại. Vui lòng thử lại sao!" + vbNewLine
                failStop()
            End If
    ...
    Public Sub makeDockerFile(path1 As String, path2 As String)
        ...
        If (maindocker.Length > 0) Then
            ...
            logContainer()
        Else
            output.Text += "Chưa thể thêm Docker vào dự án. Vui lòng thử lại sao!" + vbNewLine + vbNewLine
            failStop()
        End If
        downScroll()
    End Sub
    ...
    Async Sub logContainer()
        ...
        ElseIf (result_shell.Contains("Login Succeeded")) Then
            ...
            dockerbuild()
        Else
            output.Text += "Không thể tiếp tục thực hiện, có thể bạn chưa cài Heroku hoặc Docker Desktop (hoặc Docker Desktop chưa được chạy)" + vbNewLine + "Vui lòng thử lại sao!" + vbNewLine
            output.Text += result_shell + vbNewLine
            failStop()
        End If
        downScroll()
...
"))
                If (result_shell.Contains(appName.Text)) Then
                    ...
                    dockerRun()
                Else
                    output.Text += "Không thể tiếp tục thực hiện. Không thể tạo Image Docker. Vui lòng thử lại sao!" + vbNewLine
                    ' output.Text += result_shell + vbNewLine
                    failStop()
                End If
            Else
                output.Text += "Không thể thực hiện tiếp tục, có thể bạn chưa cài hoặc khởi động Docker Desktop!" + vbNewLine
                output.Text += result_shell + vbNewLine
                failStop()
            End If
    ...
    Async Sub dockerRun()
        ...
        ElseIf (result_shell.Contains("already allocated")) Then
            ...
            failStop()
        Else
            output.Text += "Gặp sự cố khi chạy Image '" + appName.Text + "' trên Linux Docker Desktop. Vui lòng thử lại sao!" + vbNewLine
            output.Text += result_shell + vbNewLine
            failStop()
        End If
        downScroll()
...
"))
    ...
    Async Sub pushHeroku()
        ...
        ElseIf (result_shell.Contains("Pushing web")) Then
            ...
            publicWEB()
        Else
            output.Text += "Gặp sự cố không xác định khi push web của bạn lên Heroku! Vui lòng thử lại sao!" + vbNewLine
            output.Text += result_shell + vbNewLine
            failStop()
        End If
        downScroll()
